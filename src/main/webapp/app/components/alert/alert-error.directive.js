(function() {
    ...
    function jhiAlertErrorController ($scope, AlertService, $rootScope) {
        ...
        var cleanHttpErrorListener = $rootScope.$on('tasker0App.httpError', function (event, httpResponse) {
            var i;
            ...
            case 400:
                var errorHeader = httpResponse.headers('X-tasker0App-error');
                var entityKey = httpResponse.headers('X-tasker0App-params');
                if (errorHeader) {
